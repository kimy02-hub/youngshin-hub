/* DMS Mailbox Dashboard á app.js  v4 */
...
function addManualTask() {
  ...
  showToast(isToday?'? Auto-flagged Ñ due today!':'? Task added!');
}
...
function editTaskCC(taskId) {
  ...
  wrap.innerHTML=`<div class="cc-edit-wrap"><span style="font-size:.69rem;color:var(--text-tertiary)">?</span>
    <input class="task-cc-input" id="cc-input-${taskId}" value="${esc(t.cc||'')}" placeholder="add ccÉ"
...
function renderSubtaskEditor(t) {
  ...
  subs.forEach((s,i) => {
    ...
    row.innerHTML = `
      ...
      <input class="subtask-input" value="${esc(s.text)}" placeholder="Sub-taskÉ"
...
function buildTaskCard(task, idx) {
  ...
  const notePreview = hasNote?`<div class="task-note-preview">${esc(task.note.trim().substring(0,80))}${task.note.length>80?'É':''}</div>`:'';

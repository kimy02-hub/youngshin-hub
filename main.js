...
async function loadEmails() {
  try {
    ...
    allEmails = (data.emails || []).filter(e => !localUnflagged.has(e.id));
...
function renderRelatedEmailsEditor(t) {
  ...
  (t.relatedEmails || []).forEach(r => {
    ...
    sad.innerHTML=_attHtmlForSubtask(editingTaskId,i,s.attachments);
    list.appendChild(sad);
  });
}
...
// -- ATTACHMENTS -----------------------------------------------
function addAttachmentLink(taskId, subtaskIdx) {
  var url = prompt('Enter URL (Google Drive, SharePoint, Dropbox, etc.):');
  if (!url) return;
  if (!url.startsWith('http')) url = 'https://' + url;
  var name = prompt('Label for this link (optional):') || url;
  saveAttachment(taskId, subtaskIdx, {type:'link', name:name, url:url, id:'att_'+Date.now()});
}

function addAttachmentFile(taskId, subtaskIdx) {
  ...
  input.onchange = function() {
    ...
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) { alert('File too large. Max 2MB. For larger files, upload to Google Drive and use Add Link instead.'); return; }
    var reader = new FileReader();
    reader.onload = function(e) {
      saveAttachment(taskId, subtaskIdx, {type:'file', name:file.name, data:e.target.result, size:file.size, id:'att_'+Date.now()});
    };
...
function saveAttachment(taskId, subtaskIdx, att) {
  var t = tasks.find(function(t){ return t.id === taskId; });
  if (!t) return;
  if (subtaskIdx === null || subtaskIdx === undefined) {
    // Task-level attachment
    if (!t.attachments) t.attachments = [];
    t.attachments.push(att);
  } else {
    // Subtask-level attachment
    if (!t.subtasks || !t.subtasks[subtaskIdx]) return;
    if (!t.subtasks[subtaskIdx].attachments) t.subtasks[subtaskIdx].attachments = [];
    t.subtasks[subtaskIdx].attachments.push(att);
  }
  saveTasks();
  renderSubtaskEditor(t);
  renderAttachments(taskId);
...
function deleteAttachment(taskId, subtaskIdx, attId) {
  var t = tasks.find(function(t){ return t.id === taskId; });
  if (!t) return;
  if (subtaskIdx === null || subtaskIdx === undefined) {
    t.attachments = (t.attachments || []).filter(function(a){ return a.id !== attId; });
  } else {
    if (t.subtasks && t.subtasks[subtaskIdx]) {
      t.subtasks[subtaskIdx].attachments = (t.subtasks[subtaskIdx].attachments || []).filter(function(a){ return a.id !== attId; });
    }
  }
  saveTasks();
  renderSubtaskEditor(t);
...
function renderAttachments(taskId) {
  var container = document.getElementById('taskAttachments');
  if (!container) return;
  var t = tasks.find(function(t){ return t.id === taskId; });
  if (!t) return;
  var atts = t.attachments || [];
  container.innerHTML = atts.map(function(a) {
    var icon = a.type === 'link' ? '🔗' : '📎';
    var content = a.type === 'link'
      ? '<a href="' + a.url + '" target="_blank" class="att-link">' + icon + ' ' + escHtml(a.name) + '</a>'
      : '<a href="' + a.data + '" download="' + escHtml(a.name) + '" class="att-link">' + icon + ' ' + escHtml(a.name) + '</a>';
    return '<div class="att-item">' + content + '<button class="att-del" onclick="deleteAttachment('' + taskId + '', null, '' + a.id + '')">&times;</button></div>';
  }).join('');
}
...
function toggleColorPicker(taskId) {
  ...
  document.querySelectorAll('.color-dots.cp-open').forEach(function(p) {
    if (p.id !== 'cp-' + taskId) p.classList.remove('cp-open');
  });
  var picker = document.getElementById('cp-' + taskId);
  if (!picker) return;
  picker.classList.toggle('cp-open');
}

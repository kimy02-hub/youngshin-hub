...
function openEditTask(taskId) {
  ...
  setTimeout(function(){_renderAttList(taskId);},30);
...
function renderSubtaskEditor(t) {
  ...
  (t.subtasks || []).forEach((s, i) => {
    ...
    _sd.innerHTML=_subAttHtml(editingTaskId,i,s.attachments||[]);
...
// -- ATTACHMENTS -----------------------------------------------
function addAttachmentLink(taskId, subIdx) {
  var url = prompt('Paste URL (Google Drive, SharePoint, Dropbox, etc.):');
  if (!url) return;
  if (url.indexOf('http') !== 0) url = 'https://' + url;
  var label = prompt('Label (leave blank to use URL):') || url;
  _saveAtt(taskId, subIdx, {type:'link', name:label, url:url, id:'a'+Date.now()});
}
function addAttachmentFile(taskId, subIdx) {
  ...
  inp.onchange = function() {
    ...
    r.onload = function(e) { _saveAtt(taskId, subIdx, {type:'file', name:f.name, data:e.target.result, id:'a'+Date.now()}); };
    r.readAsDataURL(f);
  };
  inp.click();
}
function _saveAtt(taskId, subIdx, att) {
  ...
  if (subIdx===null || subIdx===undefined) {
    ...
    if (!t.subtasks || !t.subtasks[si]) return;
    if (!t.subtasks[si].attachments) t.subtasks[si].attachments = [];
    t.subtasks[si].attachments.push(att);
  }
  saveTasks();
  var te = tasks.find(function(x){ return x.id===editingTaskId; });
  if (te) { renderSubtaskEditor(te); _renderAttList(taskId); }
}
function _deleteAtt(taskId, subIdx, attId) {
  var t = tasks.find(function(x){ return x.id===taskId; });
  if (!t) return;
  if (subIdx===null || subIdx===undefined) {
    ...
    var si = parseInt(subIdx);
    if (t.subtasks && t.subtasks[si]) t.subtasks[si].attachments = (t.subtasks[si].attachments||[]).filter(function(a){ return a.id!==attId; });
  }
  saveTasks();
  var te = tasks.find(function(x){ return x.id===editingTaskId; });
  if (te) { renderSubtaskEditor(te); _renderAttList(taskId); }
}
function _renderAttList(taskId) {
  var el = document.getElementById('taskAttachmentList');
  if (!el) return;
  var t = tasks.find(function(x){ return x.id===taskId; });
  ...
  el.innerHTML = atts.map(function(a) {
    ...
    var tid = taskId.replace(/'/g,''); var aid = a.id.replace(/'/g,'');
    return '<div class="att-row">'+lnk+'<button class="att-x" onclick="_deleteAtt('+chr(39)+tid+chr(39)+',null,'+chr(39)+aid+chr(39)+')">&#215;</button></div>';
  }).join('');
}
function _subAttHtml(taskId, si, atts) {
  atts = atts || [];
  var tid = taskId ? taskId.replace(/'/g,'') : '';
  var rows = atts.map(function(a) {
    var icon = a.type==='link' ? '&#128279;' : '&#128206;';
    var lnk = a.type==='link' ? '<a href="'+a.url+'" target="_blank" class="att-lnk">'+icon+' '+a.name+'</a>' : '<a href="'+a.data+'" download="'+a.name+'" class="att-lnk">'+icon+' '+a.name+'</a>';
    var aid = a.id.replace(/'/g,'');
    return '<span class="att-row">'+lnk+'<button class="att-x" onclick="_deleteAtt('+chr(39)+tid+chr(39)+','+si+','+chr(39)+aid+chr(39)+')">&#215;</button></span>';
  }).join('');
  return '<div class="sub-att">'+(rows?'<div class="att-rows">'+rows+'</div>':'')
    +'<button class="att-sm" onclick="addAttachmentLink('+chr(39)+tid+chr(39)+','+si+')">&#128279;</button>'
    +'<button class="att-sm" onclick="addAttachmentFile('+chr(39)+tid+chr(39)+','+si+')">&#128206;</button>'
    +'</div>';
}
